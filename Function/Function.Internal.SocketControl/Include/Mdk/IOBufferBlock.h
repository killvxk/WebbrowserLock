...
//写入方法
	...
	*/
	unsigned char* PrepareBuffer( unsigned long uLength );
	/**
	 * 写入完成
	 * 标记写入操作写入数据的长度
	 * 必须与PrepareBuffer()成对调用
	 */
	void WriteFinished(unsigned long uLength);
...
//读取方法
	...
	//冲缓冲块读取uLength长度的数据
	//返回实际读取的数据长度
	unsigned long ReadData( unsigned char *data,unsigned long uLength, bool bDel = true );
};
