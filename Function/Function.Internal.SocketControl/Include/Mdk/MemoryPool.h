...
private:
	...
	unsigned long m_uMemorySize;
	//管理的内存数
	int32 m_uMemoryCount;
	//未分配内存数
	int32 m_uFreeCount;
	void *m_resizeCtrl;
...
public:
	MemoryPool();
	MemoryPool( unsigned long uMemorySize, unsigned long uMemoryCount );
	//析构函数，递归的释放自身和其后所有内存池
	~MemoryPool();
	//初始化内存池
	bool Init(unsigned long uMemorySize, unsigned long uMemoryCount);
...
private:
	...
	//取得地址指向内存在块中序号(链表方法)
	unsigned long GetMemoryIndex(unsigned char* pObj);
