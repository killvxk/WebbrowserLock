...
SOCKET NetEngine::Connect(const char* ip, int port)
{
	uint64 addr64 = 0;
...
//向某主机发送消息(业务层接口)
bool NetEngine::SendMsg( int hostID, char *msg, int msgsize )
{
	bool bRet;
	AutoLock lock( &m_connectsMutex );
	...
	{
		bRet = pConnect->SendData((const unsigned char*)msg,msgsize);
	}
	pConnect->Release();//使用完毕释放共享对象
