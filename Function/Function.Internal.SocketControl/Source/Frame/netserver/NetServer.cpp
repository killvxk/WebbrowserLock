...
//连接其它IP
HOSTID NetServer::Connect(const char *ip, int port)
{
	
	return m_pNetCard->Connect(ip, port);
}
...
//向某主机发送消息
bool NetServer::SendMsg(HOSTID hostID, char *msg, int msgsize )
{
...
/*
	关闭与主机的连接
 */
void NetServer::CloseConnect(HOSTID hostID )
{
