...
unsigned char* IOBufferBlock::PrepareBuffer(unsigned long uLength)
{
	if ( 0 >= uLength || m_uLength + uLength > BUFBLOCK_SIZE ) return NULL;
	return &m_buffer[m_uLength];
}
 
void IOBufferBlock::WriteFinished(unsigned long uLength)
{
	m_uLength += uLength;
}
 
//冲缓冲块读取uLength长度的数据
unsigned long IOBufferBlock::ReadData( unsigned char *data,unsigned long uLength, bool bDel )
{
