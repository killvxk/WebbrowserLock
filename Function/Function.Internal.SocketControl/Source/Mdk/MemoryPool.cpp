...
MemoryPool::MemoryPool( unsigned long uMemorySize, unsigned long uMemoryCount )
{
	m_resizeCtrl = new Mutex;
...
//初始化内存池
bool MemoryPool::Init(unsigned long uMemorySize, unsigned long uMemoryCount)
{
...
{
	unsigned long uIndex = GetMemoryIndex( pObj );
...
unsigned long MemoryPool::GetMemoryIndex(unsigned char* pObj)
{
