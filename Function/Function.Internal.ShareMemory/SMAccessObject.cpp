...
{
	m_SMHandle = InternalShareMemory::CreateShareMemory(key, nHeadSize + nSize);
	if(SM_INVALID_HANDLE == m_SMHandle)
	{
		return FALSE;
	}

	m_pHeadData = InternalShareMemory::MapShareMemory(m_SMHandle);
	if(NULL == m_pHeadData)
	{
		return FALSE;
	}
...
void CSMAccessObject::Destory()
{
	if(m_pData != NULL)
	{
		InternalShareMemory::UnMapShareMemory(m_pData);
		m_pData = NULL;
	}

	if(m_SMHandle != SM_INVALID_HANDLE)
	{
		InternalShareMemory::CloseSharememory(m_SMHandle);
		m_SMHandle = SM_INVALID_HANDLE;
	}
...
{
	m_SMHandle = InternalShareMemory::OpenShareMemory(key, nHeadSize + nSize);
	if(SM_INVALID_HANDLE == m_SMHandle)
	{
		return FALSE;
	}
