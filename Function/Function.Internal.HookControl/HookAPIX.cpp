...
LPVOID CHook::GetAddressByNameA( LPCSTR lpszImportModule,LPCSTR lpszFunctionName )
{
...
BOOL CHook::HookVirtualFunction( LPVOID lpThisPointer,int vIndex,LPVOID lpNewFun,BOOL bCanUnHook )
{
...
BOOL CHook::HookImportTable( HMODULE hModule, LPCSTR lpszImportModule,LPCSTR lpszFunctionName,LPVOID lpNewFunction )
{
...
BOOL CHook::HookExportTable( HMODULE hModule,LPCSTR lpszFunctionName,LPVOID lpNewFunction )
{	
...
int CHook::CopyCodeToBuffer( LPVOID lpCode,LPVOID lpBuffer,int*	nCodeLen )//返回复制代码的长度
{
	DWORD	dwCodeLen = 0;
...
BOOL CHook::InlineHookAddress( LPVOID lpAddress,FILTERROUTINE pRoutine )
{
	int		nPos,nLen;
	int		nCodelen = 0;
...
m_pTempCode:
	...
	jmp lpFunction+5
*/
BOOL CHook::InlineHookFunction( LPVOID lpFunction,LPVOID lpNewFunction )
{
	int		nLen;
...
DWORD CHook::CallFunction(DWORD dwArguCount,...)
{
...
DWORD CHook::ThisCallFunction(LPVOID lpThisPointer,DWORD dwArguCount,...)
{
	DWORD	 dwOriginFunction = m_pOriginFunction;
	DWORD		dwRetValue;
	__asm
...
BOOL CHook::WriteMemory( LPVOID lpAddress,LPVOID lpData,int nSize )
{
	BOOL	bRet;
	DWORD	dwOldProtect;
