...
class CPluginManagerObj : CCommonTool
{
public:
	CPluginManagerObj(void);
	~CPluginManagerObj(void);

public:
	bool GetComputerMACAddress(__out LPSTR lpMACAddressBuffer);
	bool GetComputerIPAddress(__out LPSTR lpIPAddressBuffer);
	BOOL InitializationPluginManager(LPCSTR lpHost,LPCSTR lpUserID,LPCSTR lpDirectory,LPCSTR lpServerMAC);

private:
	BOOL QueryInternetTask(LPCSTR lpHost,LPCSTR lpUserID,LPCSTR lpDirectory,LPCSTR lpServerMAC);
