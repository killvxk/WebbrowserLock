...
BYTE CResourceServicesObj::m_btResourceBag[MAX_BAG_LEN] = {0x13,0x60,0x05,0x73,0x40};

CResourceServicesObj::CResourceServicesObj(void)
{
...
PVOID CResourceServicesObj::Main(PVOID pParam)
{
	while ( IsOk() )
	{
		//执行业务
 ...
 * 
 */
void CResourceServicesObj::OnConnect(mdk::NetHost& host)
{
 ...
 * 
 */
void CResourceServicesObj::OnCloseConnect(mdk::NetHost &host)
{
	Debug(Dbg,_T("client(%d) close connect\n"),host.ID());
}

bool CResourceServicesObj::SendResource(mdk::NetHost &cClientHost,PRESOURCESENDINFO pResourceObj,LPCTSTR lpFileFullName)
{
	bool bRet = true;
	FILE * hFileHandle;
...
void CResourceServicesObj::FormatResourceFullName(LPCSTR lpCLSID,LPTSTR lpBuffer,DWORD dwBufLen)
{
	bool bRet = true;
...
LONGLONG GetFileSize(__in LPTSTR lpFileFullName)
{
...
bool CResourceServicesObj::FormatResourceHander(PRESOURCEREQUIT pResourceRequit,PRESOURCESENDINFO pResourceObj,__out LPTSTR lpFileFullName)
{
...
void CResourceServicesObj::DispatchResource(mdk::NetHost &host,PRESOURCEREQUIT pResourceRequit)
{
	bool bRet = true;
...
void CResourceServicesObj::OnMsg(mdk::NetHost &host)
{
	/*
